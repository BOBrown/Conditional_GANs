...
#
#
# A demo CGan demo Developed by Deeparsing Studio, Version 1.
# ==============================================================================
...
def show_train_plot(hist, show = False, save = False, path = 'train_plot.png'):
...
if not os.path.isdir(root):
    os.mkdir(root)
if not os.path.isdir(root + 'all_results'):
    os.mkdir(root + 'all_results')

train_plot = {}
train_plot['D_losses'] = []
train_plot['G_losses'] = []
train_plot['per_epoch_ptimes'] = []
train_plot['total_ptime'] = []
...
for epoch in range(train_epoch):
    ...
    fixed_p = root + 'all_results/' + model + str(epoch + 1) + '.png'
    show_result((epoch + 1), save=True, path=fixed_p)
    train_plot['D_losses'].append(np.mean(D_losses))
    train_plot['G_losses'].append(np.mean(G_losses))
    train_plot['per_epoch_ptimes'].append(per_epoch_ptime)

end_time = time.time()
total_ptime = end_time - start_time
train_plot['total_ptime'].append(total_ptime)

print('Avg per epoch ptime: %.2f, total %d epochs ptime: %.2f' % (np.mean(train_plot['per_epoch_ptimes']), train_epoch, total_ptime))
print("Training finish!... save training results")
with open(root + model + 'train_plot.pkl', 'wb') as f:
    pickle.dump(train_plot, f)

show_train_plot(train_plot, save=True, path=root + model + 'train_plot.png')

images = []
for e in range(train_epoch):
    img_name = root + 'all_results/' + model + str(e + 1) + '.png'
    images.append(imageio.imread(img_name))
imageio.mimsave(root + model + 'dynamic.gif', images, fps=5)
